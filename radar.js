...
var RadarChart = {
  draw: function(id, data,allAxis, options) {
    ...
    function updateConfig() {
      ...
      config.maxValue = Math.max(config.maxValue, d3.max(data, function(group) {
        return  d3.max(group,function(d){return parseInt(d.value)});
      }));
      config.w *= config.levelScale;
      config.h *= config.levelScale;
      config.paddingX = config.w * 0.5;
      config.paddingY = config.h * 0.5;
    ...
    function buildVisComponents() {
      ...
      vis.svg = d3.select(id)
        ...
        .append("svg:g")
        .attr("transform", "translate(" + config.w*0.25 + "," + config.h*0.25 + ")");
    ...
    function buildCoordinates(data) {
      data.forEach(function(group) {
        group.forEach(function(d,i){
          d.coordinates={
            x: config.w / 2 * (1 - (parseFloat(Math.max(parseInt(d.value), 0)) / config.maxValue) * Math.sin(i * config.radians / vis.totalAxes)),
            y: config.h / 2 * (1 - (parseFloat(Math.max(parseInt(d.value), 0)) / config.maxValue) * Math.cos(i * config.radians / vis.totalAxes))
          }
        });
      });
    }
    ...
    function buildPolygons(data) {
      vis.vertices
        ...
        .attr("stroke", function(d, i) { return config.colors(i-1); })
        .attr("fill", function(d, i) { return config.colors(i-1); })
